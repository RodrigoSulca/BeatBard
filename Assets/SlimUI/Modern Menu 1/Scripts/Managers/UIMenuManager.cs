...
namespace SlimUI.ModernMenu{
	public class UIMenuManager : MonoBehaviour {
		...
		public void ReturnMenu(){
			...
			if (mainMenu != null) mainMenu.SetActive(true);      // ← todo el canvas
			if (firstMenu != null) firstMenu.SetActive(true);    // ← los botones principales
		}
		...
		public void AreYouSure(){
			Debug.Log("AreYouSure() triggered");

			if (exitMenu != null){
				Debug.Log("exitMenu is: " + exitMenu.name);
				exitMenu.SetActive(true); // ✅ muestra el menú de confirmación
			} else {
				Debug.LogWarning("exitMenu is NULL");
			}

			if (firstMenu != null){
				firstMenu.SetActive(false); // ✅ oculta solo los botones principales (MAIN), no el canvas completo
			}

			if (extrasMenu != null){
				extrasMenu.SetActive(false); // por si se abrió antes
			}

			DisablePlayCampaign(); // oculta PLAY si estaba abierto
		}
